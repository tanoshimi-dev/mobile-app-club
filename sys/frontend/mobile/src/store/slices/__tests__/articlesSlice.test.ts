...
describe('articlesSlice', () => {
  describe('reducers', () => {
    ...
    it('should handle updateArticleInList', () => {
      const existing = {...initialState, articles: [{...mockArticle}]};
      const updated = {...mockArticle, title: 'Updated Title'};
      const state = articlesReducer(existing, updateArticleInList(updated));
      expect(state.articles[0].title).toBe('Updated Title');
    });

    it('should not update if article not found', () => {
      const existing = {...initialState, articles: [{...mockArticle}]};
      const nonExistent = {...mockArticle, id: 999, title: 'Other'};
      const state = articlesReducer(existing, updateArticleInList(nonExistent));
  ...
  describe('fetchTrendingArticles', () => {
    it('should set loading on pending', () => {
      const action = {type: fetchTrendingArticles.pending.type};
      const state = articlesReducer(initialState, action);
      expect(state.loading).toBe(true);
    });
    ...
    it('should set error on rejected', () => {
      const action = {type: fetchTrendingArticles.rejected.type, payload: 'Failed'};
      const state = articlesReducer(initialState, action);
      expect(state.error).toBe('Failed');
    });
  });
  ...
  describe('fetchSavedArticles', () => {
    it('should set loading on pending', () => {
      const action = {type: fetchSavedArticles.pending.type};
      const state = articlesReducer(initialState, action);
      expect(state.loading).toBe(true);
    });
    ...
    it('should set error on rejected', () => {
      const action = {type: fetchSavedArticles.rejected.type, payload: 'Failed'};
      const state = articlesReducer(initialState, action);
      expect(state.error).toBe('Failed');
    });
  });
  ...
  describe('fetchArticleDetail', () => {
    it('should set loading on pending', () => {
      const action = {type: fetchArticleDetail.pending.type};
      const state = articlesReducer(initialState, action);
      expect(state.loading).toBe(true);
    });
    ...
    it('should set error on rejected', () => {
      const action = {type: fetchArticleDetail.rejected.type, payload: 'Failed'};
      const state = articlesReducer(initialState, action);
      expect(state.error).toBe('Failed');
    });
  });
  ...
  describe('searchArticles', () => {
    it('should set loading on pending', () => {
      const action = {type: searchArticles.pending.type};
      const state = articlesReducer(initialState, action);
      expect(state.loading).toBe(true);
    });
    ...
    it('should set error on rejected', () => {
      const action = {type: searchArticles.rejected.type, payload: 'Failed'};
      const state = articlesReducer(initialState, action);
      expect(state.error).toBe('Failed');
    });
  });
  ...
  describe('likeArticle', () => {
    it('should update like status and count in articles list', () => {
      const existing = {...initialState, articles: [{...mockArticle}]};
      const action = {
    ...
    it('should update currentArticle if matching', () => {
      ...
      expect(state.currentArticle?.like_count).toBe(6);
    });

    it('should update trending articles', () => {
      const existing = {...initialState, trendingArticles: [{...mockArticle}]};
      const action = {
        type: likeArticle.fulfilled.type,
        payload: {articleId: 1, likeCount: 6},
      };
      const state = articlesReducer(existing, action);
      expect(state.trendingArticles[0].is_liked).toBe(true);
    });
  });
  ...
  describe('unlikeArticle', () => {
    ...
    it('should update currentArticle if matching', () => {
      const liked = {...mockArticle, is_liked: true, like_count: 3};
      const existing = {...initialState, currentArticle: liked};
      const action = {type: unlikeArticle.fulfilled.type, payload: 1};
      const state = articlesReducer(existing, action);
      expect(state.currentArticle?.is_liked).toBe(false);
      expect(state.currentArticle?.like_count).toBe(2);
    });
  });

  describe('saveArticle', () => {
    it('should set is_saved true in articles list', () => {
      const existing = {...initialState, articles: [{...mockArticle}]};
    ...
    it('should update currentArticle if matching', () => {
      ...
      const action = {type: saveArticle.fulfilled.type, payload: 1};
      const state = articlesReducer(existing, action);
      expect(state.currentArticle?.is_saved).toBe(true);
    });
    ...
    it('should update trending articles', () => {
      ...
      const action = {type: saveArticle.fulfilled.type, payload: 1};
      const state = articlesReducer(existing, action);
      expect(state.trendingArticles[0].is_saved).toBe(true);
    });
  });
  ...
  describe('unsaveArticle', () => {
    ...
    it('should update currentArticle if matching', () => {
      const saved = {...mockArticle, is_saved: true};
      const existing = {...initialState, currentArticle: saved};
      const action = {type: unsaveArticle.fulfilled.type, payload: 1};
      const state = articlesReducer(existing, action);
      expect(state.currentArticle?.is_saved).toBe(false);
    });
  });
});
